package com.example.frequencyapp.audio.calculators;
...
public class RealDoubleFFT {
        ...
        if (ido != 2) {
            for (k = 0; k < l1; k++) {
                for (i = 2; i < ido; i += 2) {
                    ic = ido - i;
                    int i1 = i - 1 + (k + l1) * ido;
                    tr2 = wtable[i - 2 + iw1] * cc[i1] + wtable[i - 1 + iw1] * cc[i + (k + l1) * ido];
                    ti2 = wtable[i - 2 + iw1] * cc[i + (k + l1) * ido] - wtable[i - 1 + iw1] * cc[i1];
    ...
    void radf3(int ido, int l1, final double[] cc, double[] ch,
        ...
        for (k = 0; k < l1; k++) {
            for (i = 2; i < ido; i += 2) {
                ic = ido - i;
                int i1 = i - 1 + (k + l1) * ido;
                dr2 = wtable[i - 2 + iw1] * cc[i1] + wtable[i - 1 + iw1] * cc[i + (k + l1) * ido];
                di2 = wtable[i - 2 + iw1] * cc[i + (k + l1) * ido] - wtable[i - 1 + iw1] * cc[i1];
                int i2 = i - 1 + (k + l1 * 2) * ido;
                int i3 = i + (k + l1 * 2) * ido;
                dr3 = wtable[i - 2 + iw2] * cc[i2] + wtable[i - 1 + iw2] * cc[i3];
                di3 = wtable[i - 2 + iw2] * cc[i3] - wtable[i - 1 + iw2] * cc[i2];
                cr2 = dr2 + dr3;
                ci2 = di2 + di3;
    ...
    void radf4(int ido, int l1, final double[] cc, double[] ch, final double[] wtable, int offset) {
        ...
        if (ido != 2) {
            for (k = 0; k < l1; k++) {
                for (i = 2; i < ido; i += 2) {
                    ic = ido - i;
                    int i1 = i - 1 + (k + l1) * ido;
                    cr2 = wtable[i - 2 + iw1] * cc[i1] + wtable[i - 1 + iw1] * cc[i + (k + l1) * ido];
                    ci2 = wtable[i - 2 + iw1] * cc[i + (k + l1) * ido] - wtable[i - 1 + iw1] * cc[i1];
                    int i2 = i - 1 + (k + 2 * l1) * ido;
                    int i3 = i + (k + 2 * l1) * ido;
                    cr3 = wtable[i - 2 + iw2] * cc[i2] + wtable[i - 1 + iw2] * cc[i3];
                    ci3 = wtable[i - 2 + iw2] * cc[i3] - wtable[i - 1 + iw2] * cc[i2];
                    int i4 = i - 1 + (k + 3 * l1) * ido;
                    int i5 = i + (k + 3 * l1) * ido;
                    cr4 = wtable[i - 2 + iw3] * cc[i4] + wtable[i - 1 + iw3] * cc[i5];
                    ci4 = wtable[i - 2 + iw3] * cc[i5] - wtable[i - 1 + iw3] * cc[i4];
                    tr1 = cr2 + cr4;
                    tr4 = cr4 - cr2;
                    ti1 = ci2 + ci4;
                    ti4 = ci2 - ci4;
        ...
        for (k = 0; k < l1; k++) {
            int i1 = ido - 1 + (k + l1) * ido;
            int i2 = ido - 1 + (k + 3 * l1) * ido;
            ti1 = -hsqt2 * (cc[i1] + cc[i2]);
            tr1 = hsqt2 * (cc[i1] - cc[i2]);
            ch[ido - 1 + 4 * k * ido] = tr1 + cc[ido - 1 + k * ido];
            ch[ido - 1 + (4 * k + 2) * ido] = cc[ido - 1 + k * ido] - tr1;
            int i3 = ido - 1 + (k + 2 * l1) * ido;
            ch[(4 * k + 1) * ido] = ti1 - cc[i3];
            ch[(4 * k + 3) * ido] = ti1 + cc[i3];
        }
    }
    ...
    void radf5(int ido, int l1, final double[] cc, double[] ch, final double[] wtable, int offset) {
        ...
        for (k = 0; k < l1; ++k) {
            for (i = 2; i < ido; i += 2) {
                ...
                di2 = wtable[i - 2 + iw1] * cc[i + (k + l1) * ido] - wtable[i - 1 + iw1] * cc[i1];
                int i2 = i + (k + 2 * l1) * ido;
                int i3 = i - 1 + (k + 2 * l1) * ido;
                dr3 = wtable[i - 2 + iw2] * cc[i3] + wtable[i - 1 + iw2] * cc[i2];
                di3 = wtable[i - 2 + iw2] * cc[i2] - wtable[i - 1 + iw2] * cc[i3];
                int i4 = i - 1 + (k + 3 * l1) * ido;
                int i5 = i + (k + 3 * l1) * ido;
                dr4 = wtable[i - 2 + iw3] * cc[i4] + wtable[i - 1 + iw3] * cc[i5];
                di4 = wtable[i - 2 + iw3] * cc[i5] - wtable[i - 1 + iw3] * cc[i4];
                int i6 = i - 1 + (k + 4 * l1) * ido;
                int i7 = i + (k + 4 * l1) * ido;
                dr5 = wtable[i - 2 + iw4] * cc[i6] + wtable[i - 1 + iw4] * cc[i7];
                di5 = wtable[i - 2 + iw4] * cc[i7] - wtable[i - 1 + iw4] * cc[i6];
                cr2 = dr2 + dr5;
                ci5 = dr5 - dr2;
                cr5 = di2 - di5;
                ci2 = di2 + di5;
    ...
    void rfftf1(int n, double[] c, final double[] wtable, double[] ch) {
        int i;
        int k1, l1, l2, na, kh, nf, ip, iw, ido, idl1;

        System.arraycopy(wtable, 0, ch, 0, n);
    ...
    void rfftf(int n, double[] r, double[] wtable, double[] ch) {
        if (n == 1) return;
        rfftf1(n, r, wtable, ch);
    }

    void rffti1(int n, double[] wtable) {
        ...
        while (true) {
            ...
            do {
                ...
                ++nf;
                wtable[nf + 1 + 2 * n] = ntry;

                nl = nq;
                if (ntry == 2 && nf != 1) {
                    for (i = 2; i <= nf; i++) {
                        ib = nf - i + 2;
                        wtable[ib + 1 + 2 * n] = wtable[ib + 2 * n];
                    }
                    wtable[2 + 2 * n] = 2;
                }
            } while (nl != 1);
        ...
        for (k1 = 1; k1 <= nfm1; k1++) {
            ...
            for (j = 1; j <= ipm; ++j) {
                ...
                for (ii = 3; ii <= ido; ii += 2) {
                    ...
                    arg = fi * argld;
                    wtable[i - 2 + n] = Math.cos(arg);
                    wtable[i - 1 + n] = Math.sin(arg);
                }
                is += ido;
            }
            l1 = l2;
